...
HUD_TOGGLE_HOLD_SECONDS = 1.0
HUD_TOGGLE_COOLDOWN_SECONDS = 0.8
LOW_POWER_SCALE = 0.5
COLORS = {
    5: (0, 255, 0),    
    4: (255, 255, 255),
...
def main():
    ...
    target_frame_time = 1.0 / float(FPS_TARGET)
    prev_x, prev_y = 0, 0
    ...
    hud_candidate_since = 0.0
    last_hud_toggle_time = 0.0
    while True:
        ...
        h, w, c = frame.shape
        if low_power_mode:
            small = cv2.resize(frame, (0, 0), fx=LOW_POWER_SCALE, fy=LOW_POWER_SCALE)
        ...
        if results.multi_hand_landmarks and results.multi_handedness:
            ...
            count, fingers = count_fingers(lm, hand_label)
            curr_finger_count = count
            if count == 1 and fingers[0] == 1 and fingers[1] == 0:
                if hud_candidate_since == 0.0:
                    hud_candidate_since = now
                else:
                    elapsed = now - hud_candidate_since
                    if elapsed >= HUD_TOGGLE_HOLD_SECONDS and (now - last_hud_toggle_time) >= HUD_TOGGLE_COOLDOWN_SECONDS:
                        hud_visible = not hud_visible
                        last_hud_toggle_time = now
                        hud_candidate_since = 0.0
            else:
                hud_candidate_since = 0.0
        ...
        if hud_visible:
            cv2.rectangle(frame, (0,0), (WIDTH, 100), overlay_color, -1)
